...
在复现实验层面，我们先严格对齐原论文口径：使用 \texttt{facebook/opt-1.3b}、C4/realnewslike 500 条样本、论文风格 prompt 构造、200$\pm$5 个有效 scored tokens，以及 standard KGW detector。结果显示，\texttt{Fixed Delta 2.0} 的平均 z-score 达到 10.6006，检测成功率为 96.19\%，明显高于无水印文本的 $-0.2836$ 和 0.20\%；与此同时 continuation PPL 从 3.8492 上升到 4.9195，说明强可检测性依然伴随质量代价。在此基础上，我们再提出并实现 \method{} Watermark：每一步生成时不再直接指定固定 \texttt{delta}，而是在 KL 散度预算约束下选择最大可用水印偏置，使加水印后的分布 $P'_t$ 不会过度偏离原始模型分布 $P_t$。在补充改进实验中，\method{} 的检测成功率达到 96\%，高于 \texttt{Fixed Delta 2.0} 和自适应 \texttt{delta} 的 91\%；同时 \method{} 的 Distinct-1 为 0.7057、重复率为 0.2943，均优于强水印 \texttt{Fixed Delta 3.0}。这说明本文方法在检测能力和文本质量之间取得了比固定强水印更合理的折中。
...
我们在本地 Windows + Anaconda 环境中完成复现与后续改进。正式实验环境使用 \texttt{pytorch} conda 环境，核心依赖包括 PyTorch、Transformers、datasets、scipy、pandas 和 matplotlib。模型方面，复现阶段先使用 \texttt{sshleifer/tiny-gpt2} 跑通官方 Demo，因为它下载快、显存压力小，适合排查环境和接口问题；确认生成、检测、CSV 输出和绘图流程没有问题后，先下载并使用 \texttt{facebook/opt-1.3b} 完成严格复现实验，再使用 \texttt{facebook/opt-2.7b} 进行后续改进方法比较。
...
本报告把实验分成两个层次。第一层是严格对齐原论文口径的 KGW 复现实验：使用 \texttt{facebook/opt-1.3b}、C4/realnewslike、500 条固定随机种子打乱后抽取的 prompt，采用论文风格的 prompt 构造方式，从原始文本末尾裁掉固定长度 completion，并把生成长度控制在 200$\pm$5 个有效 scored tokens。第二层才是资源受限下的改进实验：在本地较小样本和不同模型设置上比较 Adaptive Delta、\method{} 和 \fullmethod{} 的效果。
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.92\textwidth}{p{0.30\textwidth}X}
        ...
        数据集 & \texttt{C4/realnewslike}，保存为 \texttt{prompts\_c4\_realnewslike\_500\_paper\_t200.txt} \\
        正式评估样本数 & 500 条 prompt，最终得到 499 条有效结果 \\
        ...
        PPL 评分 & 使用无水印 \texttt{OPT-1.3B} 仅对 continuation 评分 \\
        硬件 & 共享服务器单卡 GPU \\
        \bottomrule
    \end{tabularx}
\end{table}

严格复现主表只使用 standard KGW detector，汇报 green fraction、z-score、detection success rate、平均 \texttt{tokens\_scored} 和 continuation PPL，用来回答“我们和原论文到底有多接近”。后续改进实验再补充 Distinct、Weighted z-score、WinMax z-score、AUC 和 TPR@FPR 等增强指标。
...
\begin{table}[H]
    \centering
    \caption{OPT-1.3B + C4/realnewslike + 200$\pm$5 tokens + standard KGW detector 的严格复现结果}
...
表 \ref{tab:main-results} 是本报告最先给出的主表，因为它和原论文口径最接近：模型换成 \texttt{OPT-1.3B}，数据集固定为 C4/realnewslike，检测器固定为 standard KGW detector，生成长度也被控制到了论文常见的 200 token 量级。结果显示，\texttt{Fixed Delta 2.0} 能把平均 z-score 从 $-0.2836$ 提高到 $10.6006$，把检出率从 0.20\% 提高到 96.19\%，同时 green fraction 从 0.2414 提高到 0.5714，这一趋势与原论文结论一致。

更重要的是，长度控制已经不再是旧报告里的主要偏差来源。无水印与有水印两条线的平均 \texttt{tokens\_scored} 分别为 203.82 和 203.96，确实落在 200$\pm$5 的控制窗口内。与此同时，\texttt{Fixed Delta 2.0} 的 continuation PPL 从 3.8492 上升到 4.9195，Distinct-1 从 0.5571 下降到 0.5374，重复率从 0.4429 上升到 0.4626，说明更强的可检测性依然伴随着可观测的质量代价。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\textwidth]{opt13b_paper_zscore.png}
    \caption{严格复现实验中 standard KGW detector 的平均 z-score 对比}
...
\begin{figure}[H]
    ...
    \caption{严格复现实验中 standard KGW detector 的 green fraction 对比}
    \label{fig:paper-green}
\end{figure}

\subsection{改进方法结果（资源受限补充实验）}

在完成严格复现之后，我们再把注意力转向改进方法。下面这组结果仍然有参考价值，但它不再承担“论文主表复现”的角色，因为它使用的是 \texttt{OPT-2.7B} 与匹配的模型辅助检测分析，更适合作为后续改进实验。

\begin{table}[H]
    \centering
    \caption{OPT-2.7B + C4/realnewslike 500 条样本上的改进方法结果}
...
从表 \ref{tab:improved-results} 可以看到，\method{} 是这组补充改进实验中最适合作为检测优先主方案的结果。它的检测成功率为 95.60\%，高于 \texttt{Fixed Delta 2.0} 的 92.80\% 和 Adaptive Delta 的 88.60\%；平均 z-score 也更高。这说明 KL 约束能够在不手动指定固定 \texttt{delta} 的情况下形成稳定的统计偏差。
...
\begin{figure}[H]
    ...
    \caption{补充改进实验中不同方法的平均 z-score 对比}
    \label{fig:zscore}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{补充改进实验中不同方法的 green fraction 对比}
    \label{fig:green}
\end{figure}
...
这里需要特别说明：\method{} 和 \method{} + Weighted Detector 在改进结果表中的文本质量指标完全相同，是因为它们使用的是同一批生成文本。Weighted Detector 不改变生成结果，只改变检测方式。因此它的价值不体现在 Distinct 或普通 z-score 上，而体现在 weighted z-score、AUC 和 TPR@FPR 上。实验中 \method{} + Weighted Detector 的 weighted AUC 达到 1.0000，TPR@1\%FPR 和 TPR@5\%FPR 均为 100\%，说明模型辅助加权检测能够很好地区分无水印文本和水印文本。
...
首先，严格复现实验已经把此前最明显的口径偏差补上了：主表前置为 \texttt{OPT-1.3B + C4/realnewslike + 200$\pm$5 tokens + standard KGW detector}，并且两条线的平均 \texttt{tokens\_scored} 都稳定在 204 左右。这个结果更适合回答“我们是否真的复现了原论文”的问题，也说明此前与论文差距较大的重要原因，确实包括模型规模、prompt 构造方式和生成长度控制不一致。

在此基础上，补充改进实验支持了 \method{} 的设计方向。在 OPT-2.7B 和 C4/realnewslike 100 条样本上，\method{} 的检测成功率达到 96\%，高于 \texttt{Fixed Delta 2.0} 和旧 Adaptive Delta 的 91\%；同时它在 Distinct-1 和重复率上优于 \texttt{Fixed Delta 3.0}。Weighted Detector 进一步给出了 AUC 和低误报率下的检测优势。完整 \fullmethod{} 虽然普通检测率较低，但质量指标最好，说明 candidate-aware greenlist 和 confidence gate 提供了另一种可控折中。
