...
def main() -> None:
    ...
    config = {"detector_id": "standard_kgw", "gamma": args.gamma, "hash_key": args.hash_key, "seeding_scheme": "simple_1", "tokenizer": args.tokenizer_name_or_path}
    detector_id, detector_hash = detector_config_id(config), config_hash(config)
    manifest = {str(row["source_id"]): row for row in read_jsonl(args.manifest_jsonl) if row["split"] == args.split}
    ...
    for source_id, record in manifest.items():
        score = standard_kgw_score(record["prompt_token_ids"], record["human_completion_token_ids"], len(tokenizer), args.gamma, args.hash_key)
    ...
    for record in generations:
        ...
        score = standard_kgw_score(record["prompt_token_ids"], record["continuation_token_ids"], len(tokenizer), args.gamma, args.hash_key)
